...
def assert_inventory_columns(page):
    # Accepted design uses inventory container size, not just viewport width.
    inventory = page.locator('#credential-inventory')
    size = inventory.evaluate('''e => {
        const s = getComputedStyle(e);
        return e.clientWidth - parseFloat(s.paddingLeft) - parseFloat(s.paddingRight);
    }''')
    ...
    if size > 540 and width > 760:
        expected.append('Version')
    table = page.locator('#surface-list')
    expect(table.get_by_role('columnheader')).to_have_text(expected)
    # Assert matching cells, too: hiding headers alone conceals a broken table.
    cells = table.locator('tbody tr').first.locator('th:visible, td:visible')
    expected_cells = ['provider/testsecret-00']
    if 'Kind' in expected:
        expected_cells.append('provider-token')
    expected_cells.append('active')
    if 'Version' in expected:
        expected_cells.append('v80')
    expect(cells).to_have_text(expected_cells)
...
with sync_playwright() as p:
    ...
    for width in [1440, 900, 390]:
        ...
        expect(detail.get_by_role('columnheader')).to_have_text(['Version', 'Created', 'Stored value'])
        if width > 900:
            assert_inventory_columns(page)
        ...
        context.close()
    # Boundary and resize coverage: all four columns must return as space grows.
    context = browser.new_context(viewport={'width': 1440, 'height': 900})
    context.route('**/*', lambda route: route.continue_() if route.request.url.startswith(base + '/') else route.abort())
    page = context.new_page()
    page.goto(base + '/console/credentials?q=provider&status=active#/credentials')
    for width in [901, 900, 761, 760, 600, 500, 390, 1440]:
        page.set_viewport_size({'width': width, 'height': 900})
        assert_inventory_columns(page)
    # Explicit container thresholds independent of the surrounding shell width.
    inventory = page.locator('#credential-inventory')
    for size in [541, 540, 421, 420, 541]:
        inventory.evaluate('(e, size) => { e.style.width = size + "px"; e.style.boxSizing = "content-box"; }', size)
        assert_inventory_columns(page)
    print('PASS responsive columns: viewport 760/761, container 420/421 and 540/541, resize restoration, visible headers and cells')
    context.close()
