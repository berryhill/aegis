...
Task and acceptance are nonempty and bounded to 32768 bytes each. The default mode permits one or two passes. Set `"decision_mode": "doer.v1"` inside `implementation` to allow one to three passes under a local Laya gate and judgment; `"max_passes": 3` is rejected without that mode. The mode and all other contract fields are covered by the operator-allowlisted digest. Use timeout 1–120 seconds, explicit local non-test Go source paths, local Go package selectors, and exact package import path/top-level test-name identities from the trusted existing tests. Test files are not writable. For later contiguous revisions include the exact `previous_digest`; the builder carries it into both predecessor fields. No shell command is accepted in this contract.
...
Online validation resolves the named latest enabled registered Agent under authenticated owner authority. Match the exact revision digest on readback and require lifecycle `draft` with no lifecycle history. Publication is inactive and creates no Queue item. Activation and Queue execution require separate authenticated operations and runtime authorization; neither follows from publication. Do not stop a daemon or fall back to opening its stores.
...
For `doer.v1`, the operator must separately configure absolute `implementation.laya_python` (an interpreter with the local `laya` package) and `implementation.laya_home` (an existing private mode-0700 directory with a preinstalled checkpoint/cache). An unset or partially configured pair denies the mode; it is not installed or downloaded by Loop publication. The fixed embedded helper receives a bounded JSON task or report through stdin and returns typed decisions. It receives a minimal environment, no controller credential variables, and no shell command, but Python, the model and native Go tests are not host or network sandboxes. The decision backend fails closed on missing, low-confidence, malformed or ambiguous answers; it does not select authority. Within the existing one Queue claim, a negative task gate records `needs_input` with zero implementation passes and a failed Queue disposition reason `implementation_needs_input`. A positive gate permits a bounded Hermes patch proposal with a report; Laya judges that report, the controller runs the precommitted native Go tests regardless of judgment, and a failed pass supplies both results to a tool-free Hermes diagnosis turn before another pass. Only independent native-check evidence and all required judgment fields together permit completion. The terminal Hermes assessment is non-authoritative and its failure does not change the verified result. Exact stage payloads are retained in content-addressed implementation custody; Queue retry/attempt numbers are separate from the internal pass count.

This is a Doer-style mode of the existing v3 `implement` action, not a general interpreter for arbitrary action/gate/join/cycle graphs. It uses the same sealed Hermes model/provider for proposal and diagnosis/completion turns; it does not implement Doer's arbitrary file-presence/text verifier, Jev alternative, distinct role-model overrides, or live-provider qualification. The terminal Queue outcome remains Aegis's durable vocabulary, not Doer's CLI exit code.
