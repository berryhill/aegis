...
| Development and release binaries collide or reset the wrong authority | Build-version-derived profiles; production `~/.argis`; ignored repository-local `.aegis` after executable-directory module/worktree verification; production-path denial; separate deployment identities; reset restricted to its exact profile layout | A deliberately modified binary can impersonate release build metadata; profile separation and Git ignore are not a host sandbox |
