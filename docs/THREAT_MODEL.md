...
| Credential is pasted into manager chat | Aegis owns terminal input; bounded deterministic scanning blocks before gateway/proxy and records no content; a clear natural create request is reduced locally to validated metadata, while any inline value is discarded and must be entered again through confirmed no-echo intake outside the model path | False negatives and Go/runtime memory copies remain possible; deterministic intent parsing proposes but never authorizes a mutation |
