...
| Model/runtime output rewrites the terminal or forges Aegis approval chrome | Closed origin-classified presentation events; contextual CSI/OSC/DCS/APC/PM/SOS, control, bidi/invisible and malformed-UTF-8 sanitization before width/layout; bounded components; authoritative trust/approval fields never come from model events | Ordinary sanitized prose can still attempt social deception; the terminal and same-account process are not a sandbox |
