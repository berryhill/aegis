...
| Credential is created or explicitly retrieved in the authenticated manager | Principal authentication, deterministic low-ambiguity intent/exact-reference parsing, fail-closed blocking of explicit credential-value syntax when create grammar does not map it, direct typed authority execution without a model round trip, atomic insert-only create with duplicate-reference conflict, separately confirmed rotation/replacement, encrypted durable storage, active/revoked checks, terminal-control escaping, metadata-only audit, and retained-session-content purge | The authenticated terminal and active same-account processes can observe retrieved plaintext; terminal scrollback, swap, core dumps, kernel/root capture, filesystem snapshots, and forensic zeroization are outside Aegis's guarantee |
