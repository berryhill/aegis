...
| A certification turn stalls or emits events after cancellation | Configured per-turn deadlines, principal-expiry overall deadline, up to ten fresh executions only for missing required conversational content, default immediate abort on other failures, cancellation/expiry stop even in explicit continue-on-error diagnostic mode, permanently poisoned interrupted gateway client, reverse-order cleanup, and atomic all-cases-success-only certification publication | Slow local hardware or repeated incomplete conversational responses can exhaust short principal authority and require an explicit retry; diagnostic continuation costs additional local resources; Hermes events do not carry prompt correlation IDs |
