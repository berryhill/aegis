...
| Onboarding silently selects, downloads, or mutates a model | Closed no-default registry, loopback installed-only Ollama discovery, immutable digest match, complete preview, literal confirmation, configuration-digest recheck, strict validation, and atomic mode-0600 publication; certification and activation remain separate | Ollama metadata and the local operator remain trust dependencies; Aegis does not prove artifact provenance merely from a local digest |
| Reset follows a malicious path or erases external/operator state | Host-native principal match, operator-home containment, repository/root/home rejection, no symlink following, owner/mode/link checks, closed artifact inventory, Aegis schema evidence for local authority files, complete preview, exact phrase, immediate full-plan revalidation, empty-directory-only removal, and configuration-last ordering | Same-account races cannot be made a filesystem transaction across independent roots; a mid-apply I/O failure is reported as `reset_incomplete`, configuration remains when failure occurs before its final deletion, and backups are required for recovery |
