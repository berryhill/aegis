...
| A certification turn stalls or emits events after cancellation | Configured per-turn deadlines, principal-expiry overall deadline, immediate abort on operational or invalid-envelope failures, at most three fresh executions only for missing required conversational content, permanently poisoned interrupted gateway client, reverse-order cleanup, and atomic success-only certification publication | Slow local hardware or repeated incomplete conversational responses can exhaust short principal authority and require an explicit retry; Hermes events do not carry prompt correlation IDs |
