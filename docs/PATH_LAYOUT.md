...
Linux development recovery: when the exact executable-selected `<repository>/.aegis/aegis.yaml` is absent, `./aegis reset` can preview and, after authenticated OS-operator confirmation, remove only `<repository>/.aegis/state/transport/aegis.sock`. The socket must be single-link, operator-owned, protected against other-user writes, absent by VFS device/inode identity from a complete Linux UNIX socket diagnostic dump (all states, including non-listening relative/alias binds), and return exactly connection-refused. Secure no-symlink ancestors and socket identities are digest-bound and revalidated; deletion is descriptor-anchored. Apply acquires the same persistent `aegis.sock.lock` flock as API startup, validates its no-follow regular-file owner/mode/link/device/inode identity, and holds it through fresh socket checks, removal, directory sync and absence readback. The coordination file is created if missing and retained, never removed. Unavailable, truncated, interrupted or malformed kernel diagnostics fail closed. Live, bound, foreign, symlinked, changed, and otherwise unverified transports deny. No other state is inferred safe to delete from missing configuration. Production passphrase authority and valid-config gateway controls are unchanged. Bootstrap itself still never deletes transport. Reset completion additionally requires the next bootstrap socket to be absent. This is same-account local lifecycle recovery, not protection against malicious same-UID/root processes or shared cross-namespace state.
