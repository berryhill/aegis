...
## 12. Complete the remaining fleet-v1 worker lifecycle and public application wiring

Scope: build on the implemented strict queue/execution/evidence/disposition records, authenticated internal Graph submission, atomic accepted-or-rejected submission, dependency-gated attempt-bounded claims, durable expired-lease retry/reclaim, queued-or-claimed cancellation, validated rebuildable queue projections, and narrow single-node worker with fresh claim/runtime/evidence/disposition admission. Add public application/transport wiring, a real Hermes-backed worker adapter, automated reclaim/retry/cancellation scheduling, dirty-store recovery, multi-node scheduling, lifecycle tooling, and installed proof. Relevant files: `internal/persistence/qualification`, `internal/persistence/fleet`, `internal/registry`, `internal/loop`, `internal/graph`, `internal/queue`, `internal/execution`, `internal/evidence`, `internal/disposition`, `internal/orchestration`, and future shared application adapters. Acceptance: exclusive writer-lock and reserve enforcement; clean/dirty reopen and recovery; concurrent scheduler, cancellation, retry-exhaustion, migration, backup/restore, and full canonical projection-rebuild campaigns; exact Hermes runtime identity; and installed Registry → Loop → Graph → Queue → evidence-gated disposition proof. Security: preserve the separate session-authority database, repeat fresh admission at every runtime effect, derive scheduling from replay-verified canonical lifecycle rather than projection or process liveness alone, deny alternate paths/schemas and partial readiness, and do not represent the narrow worker as complete crash-safe general Graph execution.
