...
## 12. Complete the remaining fleet-v1 queue lifecycle and application wiring

Scope: build on the implemented strict queue/execution codecs, authenticated internal Graph submission admission, atomic accepted-or-rejected submission transaction, and single-winner initial claim/attempt transaction at the qualified `state/persistence/fleet-v1` root. Add public application/transport wiring, fresh claim/reclaim and runtime-effect admission, later queue transitions, lease expiry/recovery, retries, cancellation, terminal outcomes, evidence-gated disposition, and lifecycle tooling. Relevant files: `internal/persistence/qualification`, `internal/persistence/fleet`, `internal/registry`, `internal/loop`, `internal/graph`, `internal/queue`, `internal/execution`, `internal/orchestration`, and future shared application adapters. Acceptance: fresh authority admission at claim and launch in addition to the existing submission boundary; exclusive writer-lock and reserve enforcement; clean/dirty reopen, lease recovery, cancellation, retry exhaustion, migration, backup/restore, and projection-replay campaigns; installed Registry → Loop → Graph → Queue proof. Security: preserve the separate session-authority database, deny alternate paths/schemas and partial readiness, never infer readiness from process liveness or a persisted claim, and do not represent the initial persistence slice as complete crash-safe queue execution.
