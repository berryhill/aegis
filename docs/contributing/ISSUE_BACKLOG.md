...
## 12. Complete the qualified fleet-v1 queue and lifecycle protocol

Scope: extend the implemented narrow Badger adapter for immutable Agent/Loop/Graph/validation/snapshot/audit facts with durable submission disposition, queue transitions, claims/leases, attempts, terminal outcomes, and lifecycle tooling at the qualified `state/persistence/fleet-v1` root. Relevant files: `internal/persistence/qualification`, future `internal/persistence/fleet`, `internal/registry`, `internal/loop`, `internal/graph`, `internal/queue`, and `internal/execution`. Acceptance: exact schema and digest codecs; one-transaction snapshot plus rejection-or-queue publication; atomic single-winner claims; fresh authority admission at claim and launch; exclusive writer-lock and reserve enforcement; clean/dirty reopen, lease recovery, cancellation, retry exhaustion, migration, backup/restore, and projection-replay campaigns; installed Registry → Loop → Graph → Queue proof. Security: preserve the separate session-authority database, deny alternate paths/schemas and partial readiness, never infer readiness from process liveness, and do not claim the qualification contract alone implements crash-safe queue behavior.
