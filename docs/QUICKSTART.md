...
That four-target result is packaging evidence only. The post-Track-A persistence contract is qualified on Linux/amd64/ext4 with exact Badger `v4.9.5` session-authority, fleet-definition, and fleet-lifecycle settings plus bbolt `v1.5.0` credential-custody settings; see `specs/STORAGE.md`. The two fleet planes use one `fleet-v1` store at `state/persistence/fleet-v1`. Its internal adapter atomically persists either an accepted snapshot/submission/queue-item/Graph-run bundle or a durable rejection; dependency-gates and attempt-bounds claims; durably reclaims expired leases into bounded backoff; cancels queued or claimed work; validates rebuildable queue projections against canonical transitions; and atomically records terminal evidence metadata, disposition, transition, projection, and audit. A narrow internal worker proves one exact single-node no-key execution with fresh admission at each consequential boundary. That worker is not wired to the CLI, API, or console; it is not a real Hermes adapter and does not automate reclaim/retry/cancellation scheduling, dirty-store recovery, multi-node scheduling, or installed end-to-end fleet proof. Do not infer storage qualification for macOS, arm64, another filesystem, path, schema, writer model, or engine version from a successful cross-build.
...
The shell currently proves the authenticated transport and accessible Agent Registry list/detail/inspector primitives, including loading, empty, unavailable, and error states. The internal Badger fleet repository is tested durable substrate for immutable Registry/Loop/Graph definitions plus atomic accepted-or-rejected submission, dependency-gated claims, expired-lease reclaim/retry, cancellation, and validated queue projections. `internal/orchestration` also provides narrow authenticated, authority-bound internal registration, Loop/Graph publication, Graph submission admission, durable rejection, and contextual-readiness behavior. Neither boundary is wired to this shell or to public fleet CLI/API routes. The tested internal worker executes only the narrow deterministic single-node path; automated lifecycle scheduling, real Hermes worker execution, and installed end-to-end proof remain absent. Loop, Graph, and Execution Queue links remain product navigation only. The dependency-free static contract can be checked locally with `python3 scripts/console_security_test.py`; the network and denial behavior is covered by `go test ./internal/api ./internal/console -count=1`.
