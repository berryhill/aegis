...
Reset is intentionally fixed to the selected profile rather than accepting a custom `--config`. Repository paths remain denied except for the exact ignored `.aegis` subtree authorized by a development binary; reset also rejects the home directory itself, filesystem roots, paths outside the authenticated operator home, unsafe parents, symlinks, hard-linked files, unknown state content, and path/inode changes outside the narrow Badger rule below. A recognized Badger file may be replaced or created while confirmation is pending only inside an Aegis-owned Badger directory already included in the preview. Apply regenerates the closed recognized inventory and deletes using those fresh identities; unknown names, changed directory scope, and non-Badger changes still deny before deletion. Reset never accepts a force flag; non-TTY, Enter, explicit `no`, EOF, cancellation, or input other than `y`/`yes` performs no writes. After confirmation (and the second production authentication), it validates and disables/stops/removes only the exact current Aegis-owned `systemd --user` unit, reloads the user manager, verifies removal, and only then applies the reset plan. If the unit file is absent, reset proceeds only when systemd reports the exact unit not found and inactive. Foreign or drifted unit bytes, loaded/active state without the expected file, inspection failure, or purge failure leaves configuration and reset state intact.
