...
The copied valid configuration does not initialize operational authority. In a real terminal, run `./aegis --config .aegis.yaml init`, enter and confirm a new principal password through two fresh, bounded principal-specific protected-input requests, verify the displayed authenticated UID/username and exact owner-only verifier and `state/persistence/authority-v1` paths, and approve the exact initialization and compatibility-reconciliation previews. Each request uses pinentry first; a real-terminal no-echo prompt is allowed only when pinentry is unavailable or fails before protected interaction begins. Cancellation, timeout, malformed output, and failures after protected interaction remain fail-closed. This publishes only a salted scrypt verifier—never plaintext or reversible password material—and creates one secure empty authority generation only when the paths are exactly absent. You may exit the later credential/model onboarding stages if this quickstart is exercising only credential-independent commands. Non-interactive `init`, bare startup, and ordinary commands instead return `operational_authority_not_initialized` with exit status 2 and perform no mutation. Existing invalid or populated state is preserved for operator repair and is never replaced.
...
Open the exact origin and sign in with the principal password enrolled during approved initialization. The password form is the dashboard happy path. As an alternate sign-in, run `./aegis --config .aegis.yaml console` as the authenticated host principal to obtain a short-lived single-use handoff through the generated token file and authenticated Unix transport. This explicit development-proof command emits the configured origin, bootstrap, expiry, and explicit single-use/no-reusable-bearer fields; it does not launch a browser and does not reset the password. Loading the shell alone does not authenticate the browser. Do not put the bootstrap or reusable API bearer in a URL, command history, recording, or browser storage. The bootstrap expires after 30 seconds by default; the resulting volatile browser session expires after five minutes or when the source subject expires, whichever comes first. A native-form failure shows the stable reason, configured lifetimes, and alternate authenticated-host handoff command `aegis console` without reflecting the submitted value. Malformed values use `bootstrap_invalid_format`; valid-looking unknown, replayed, or expired values intentionally share `bootstrap_consumed_or_expired`. Use an HTTPS `api.console.origin` and configure both API TLS files before exposing the TCP listener beyond loopback.
...
`gateway preview` is read-only and prints the exact unit path and digest. `install` requires a real terminal, prints the complete deterministic installation preview, and asks `Install and activate this gateway? [Y/n]`; Enter, `y`, or `yes` confirms, while every other answer, EOF, cancellation, or input failure declines without mutation. It revalidates the complete approved plan immediately before mutation, refuses foreign or byte-drifted `aegis.service`, uses only `systemctl --user`, validates that systemd loaded the approved fragment, and completes only after authenticated readiness reports a current, verifiable audit projection. Activation failures identify the publish, daemon-reload, enable/start, exact-unit, or audit-current readiness phase and retain the last readiness and rollback diagnostics. Rollback removes only newly published unit bytes and reverses only active/enabled state introduced by that install, preserving any pre-existing exact unit and gateway state. Bare onboarding uses the same bounded `[Y/n]` contract after its complete transport reconciliation preview. On every later bare invocation, Aegis first verifies the exact installed bytes, loaded fragment, loaded `ExecStart` executable and full `serve --config` argv, activity, and authenticated audit-current `/readyz` without opening writable authority persistence. If the development state root was removed and recreated while the exact gateway remained loaded, recovery validates the loaded fragment and complete `ExecStart` before mutation, restarts the gateway, revalidates both identities, and only then accepts authenticated readiness. A foreign fragment, stale executable, or stale configuration denies without a lifecycle action. An exact healthy gateway presents `console`, `terminal`, and `exit` with default `exit`: for the example plaintext loopback origin, `console` obtains a single-use bootstrap and opens a random correlation-bound temporary handoff. The handoff lasts at most 15 seconds, admits one atomic claimant, keeps the bootstrap out of the URL, and exchanges it server-side. The configured console must then authenticate the delivered `HttpOnly` cookie and redirect through an exact correlation-bound same-host loopback callback before the CLI reports `browser_session_established=true`; the callback returns the browser to a clean `/console` URL and carries no identity, authority, bootstrap, cookie, or reusable bearer; `terminal` launches the same authenticated Unix-gateway client as `aegis manager`. The client receives a short-lived principal-and-stanza-bound session, repeats transport and session authorization for typed slash commands, and never opens gateway-owned persistence or falls back to direct state. Authentication expiry, revocation, or transport interruption denies and requires a fresh invocation. A mismatched or unhealthy loaded gateway denies with `aegis gateway status` as the next inspection command; dirty orphaned or corrupt closed authority also denies before bootstrap reconciliation. Browser opening uses a direct platform command with the complete correlation-only handoff URL as one argument and no shell interpretation. Bootstrap denial opens nothing. Unsupported automatic scope, listener failure, launcher failure, or timeout prints the configured manual URL and bootstrap and returns an actionable error; no reusable bearer is exposed. Explicit `aegis console` remains the non-launching bootstrap command for manual browser exchange. Start/stop/restart validate the exact current Aegis-owned unit before invoking `systemctl`; if the unit is absent, they return `gateway_not_installed`, name `aegis gateway install`, and perform no installation or gateway-state mutation. They then validate the loaded fragment and wait for observed active/inactive state; start and restart additionally wait for authenticated audit-current readiness. Neither status nor lifecycle commands enable linger; logout survival is reported only when the host already enables it. Uninstall requires confirmation, removes only the exact owned unit, and preserves configuration, state, and external credentials.

The shell provides accessible live surfaces for Agent Registry, Graphs, Loops, Execution Queue, and metadata-only Credentials. The data views remain read-only except that the Loops workspace additionally owns bounded native publication and lifecycle command forms and the Graphs workspace additionally owns bounded native compose/publish and typed-run forms. Agent, Loop, Queue, and Credentials views remain read-only; Graphs additionally provides bounded native compose/publish and typed-run forms. Credential mutation is not a browser control. The protected local Unix API exposes principal-only create, rotate, record/version revoke, exact binding, and policy-located ciphertext-backup operations at `/v1/credentials`; it requires the same bearer-plus-`SO_PEERCRED` transport boundary as other protected routes. Create/rotate bodies are strict and capped at 1 MiB, successful responses contain metadata only, and `/v1/credentials/backup` accepts only `{}` because the server selects the configured backup location. Never place credential values in argv, shell history, URLs, recordings, or reusable fixture files; use protected CLI intake or a purpose-built client that keeps request bytes in memory and clears them after the request. Typed Go view models are rendered through generated templ components; native same-origin forms and links handle password login, authenticated password rotation, alternate bootstrap exchange, sign-out, domain navigation, record inspection, and inspector close without executable client JavaScript, a CDN, Node runtime, inline executable content, or browser-side HTML construction. The retained pinned self-hosted Datastar asset remains embedded and provenance-checked for release compatibility, but the console document does not load it. Every collection state preserves its authoritative source and stable reason code and distinguishes ready, empty, denied, unavailable, degraded/repair-required, loading, and error outcomes; a failed read never asserts a zero count. Contextual action readiness is diagnostic and fail-closed: browser input cannot select identity, stanza, mandate, runtime authority, or success, and consequential actions remain disabled without exact server-owned authority context. The Credentials surface exposes binding metadata only and never resolves or renders secret values. Selecting a record opens an inspector with exact revisions and digests. Loop records include executable steps, transitions, ports, bounded retries, evidence contracts, immutable validation, exact publisher Agent/authority provenance, and append-only lifecycle history/readiness; Graph records include immutable validation facts; Queue records include the item, current rebuildable projection, parent Graph run, child Loop executions, and attempts so execution causality remains reviewable. Mutating Agent registration, Loop publication/lifecycle, and explicit deterministic Queue processing remain public CLI/API workflows rather than browser controls. The Graph composer resolves exact immutable Agent and Loop revisions from server-owned options, reloads their exact ports, and submits topology, mapping, policy, predecessor, and idempotency fields through an authenticated same-origin CSRF-protected native form. The typed-run form reloads the reviewed exact active Graph and takes an authority-bound runtime session ID plus typed values; the server derives authority from the session, canonicalizes inputs, and records either one durable rejection or one immutable snapshot and Queue item. Browser values cannot construct authority or override immutable references; exact retries return the original identities while changed replays conflict. The installed-MVI verifier exercises those CLI resources and the authenticated console from the extracted native archive. Durable retry/reclaim/cancellation primitives exist below this surface, but automated queue lifecycle scheduling, Graph lifecycle application, real Hermes worker execution, multi-node scheduling, and browser mutation workflows remain absent. Run `python3 scripts/verify-console-vendor.py`, `python3 scripts/console_security_test.py`, `go generate ./web/console`, and `git diff --exit-code -- web/console go.mod go.sum` for the source/supply-chain contract; network and denial behavior is covered by `go test ./internal/api ./internal/console -count=1`.

The Agent Registry destination additionally provides bounded native search and enabled/disabled/retired lifecycle filtering, stable Agent-ID selection, typed cards, and detail sections for fleet/source provenance, owner/accountability, runtime and charter bindings, immutable revision history, declared capabilities/policies, lifecycle, and action-specific execution readiness. When charter import readiness is `ready`, its native primary link opens the dedicated authenticated `/console/agents/charter-import` review page. That page provides a native link back to the Agent Registry and exact `aegis charter validate <charter-file.json>` and `aegis charter import <charter-file.json>` CLI guidance; denied or unavailable readiness leaves the action disabled with its reason. The browser does not execute either command, import a charter, provision an agent, or gain mutation authority. The Registry does not fabricate effective stanza authority or provisioning receipts: those fields remain explicitly unasserted unless their own authoritative records are available. The installed fleet verifier exercises exact Agent revision history, disabled-to-enabled append-only lifecycle transitions, and historical revision readback.
