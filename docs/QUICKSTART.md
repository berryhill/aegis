...
The shell provides an accessible authenticated operational Agent Registry plus live review surfaces for Graphs, Loops, Execution Queue, and metadata-only Credentials. Typed Go view models are rendered through generated templ components; native same-origin forms and links handle bootstrap exchange, sign-out, domain navigation, record inspection, and inspector close without executable client JavaScript, a CDN, Node runtime, inline executable content, or browser-side HTML construction. The retained pinned self-hosted Datastar asset remains embedded and provenance-checked for release compatibility, but the console document does not load it. Every collection state preserves its authoritative source and stable reason code and distinguishes ready, empty, denied, unavailable, degraded/repair-required, loading, and error outcomes; a failed read never asserts a zero count. Contextual action readiness is diagnostic and fail-closed: browser input cannot select identity, stanza, mandate, runtime authority, or success, and consequential actions remain disabled without exact server-owned authority context. The Credentials surface exposes binding metadata only and never resolves or renders secret values. Selecting a record opens an inspector with exact revisions and digests. Loop records include executable steps, transitions, ports, bounded retries, evidence contracts, immutable validation, exact publisher Agent/authority provenance, and append-only lifecycle history/readiness; Graph records include immutable validation facts; Queue records include the item, current rebuildable projection, parent Graph run, child Loop executions, and attempts so execution causality remains reviewable. Loop/Graph publication, authority-bound Loop lifecycle, Graph submission, durable rejection, and explicit deterministic queue processing remain public CLI/API workflows rather than browser controls; Agent registration and Agent lifecycle are the bounded browser mutation exception. The installed-MVI verifier exercises those CLI resources and the authenticated console from the extracted native archive. Durable retry/reclaim/cancellation primitives exist below this surface, but automated queue lifecycle scheduling, Graph lifecycle application, real Hermes worker execution, multi-node scheduling, and additional browser mutation workflows remain absent. Run `python3 scripts/verify-console-vendor.py`, `python3 scripts/console_security_test.py`, `go generate ./web/console`, and `git diff --exit-code -- web/console go.mod go.sum` for the source/supply-chain contract; network and denial behavior is covered by `go test ./internal/api ./internal/console -count=1`.

The Agent Registry destination additionally provides bounded native search and enabled/disabled/retired lifecycle filtering, stable Agent-ID selection, typed cards, and detail sections for fleet/source provenance, owner/accountability, runtime and charter bindings, immutable revision history, declared capabilities/policies, lifecycle, and action-specific execution readiness. When registration readiness is `ready`, the native primary link opens one dedicated authenticated workflow for an exact pre-imported canonical charter revision and bounded deterministic current-fleet JSON plus exact fleet/source identity; denied or unavailable readiness leaves the action disabled with its reason. Review is non-mutating, requires the exact charter revision and digest to exist already in the charter store, and renders the server-derived Agent ID, canonical charter digest, immutable revision/digest, source, runtime, ownership/accountability, capabilities, policies, and lifecycle. Execute reparses and revalidates those artifacts before shared-service registration and exact readback; it does not separately import the charter or provision an agent. Agent inspectors render only legal append-only enabled/disabled/retired transitions for the current exact revision, and retirement requires explicit confirmation. The browser does not provision an agent or invent mutation authority: each control is exact-origin and CSRF-protected, and the server authenticates the configured principal, validates artifact or exact revision preconditions, applies canonical idempotency/audit rules, and returns authoritative post-write readback. The Registry does not fabricate effective stanza authority or provisioning receipts: those fields remain explicitly unasserted unless their own authoritative records are available. The installed fleet verifier exercises exact Agent revision history, disabled-to-enabled append-only lifecycle transitions, and historical revision readback.
