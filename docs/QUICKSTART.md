...
The shell currently proves the authenticated transport and accessible Agent Registry list/detail/inspector primitives, including loading, empty, unavailable, and error states. The complete fleet thin vertical is available through the public CLI resources and protected `/v1/agents`, `/v1/loops`, `/v1/graphs`, and `/v1/queue` routes, not through the shell: shared services provide principal-authenticated registration, Loop/Graph publication, Graph submission, durable rejection, exact reads, and explicit deterministic queue processing. The installed-MVI verifier exercises those CLI resources from the extracted native archive. Durable retry/reclaim/cancellation primitives exist below this surface, but automated lifecycle scheduling, real Hermes worker execution, multi-node scheduling, and complete console fleet workflows remain absent; Loop, Graph, and Execution Queue links remain product navigation only. The dependency-free static contract can be checked locally with `python3 scripts/console_security_test.py`; the network and denial behavior is covered by `go test ./internal/api ./internal/console -count=1`.
