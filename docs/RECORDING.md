...
The installed-MVI verifier added to CI and release publication is non-interactive and intentionally not represented by this no-key Hermes recording. Its canonical evidence is the exit-checked output of `./scripts/verify-installed-mvi.sh`; it uses an isolated repository-local proof home and removes that proof workspace unless an explicit empty release-output directory is supplied.
