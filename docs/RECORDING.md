...
The installed-MVI verifier used by CI and the tag-triggered release workflow is non-interactive and intentionally not represented by this no-key Hermes recording. Its canonical local evidence is the exit-checked output of `./scripts/verify-installed-mvi.sh`: four release-shaped archives each constrained to exactly one root `aegis` regular file with mode `0755`, verified checksums/version, fail-closed first run, and an extracted-binary Registry/Loop/Graph/Queue/evidence/disposition proof with durable rejection and historical readback. It uses and removes an isolated repository-local proof home even when an explicit empty archive-output directory is retained. This does not make the historical recording current and does not prove release publication.
