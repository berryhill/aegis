...
- A complete or externally supported general Agent Registry, Loop, Graph, or Execution Queue product. The narrow public CLI/API vertical, authority-bound Loop activation/retirement, bounded Hermes gateway execution, live console inspection, reviewed Loop and Graph command forms, and closed principal-authenticated Queue process/reclaim/terminal controls exist. These browser paths derive authority server-side, require same-origin CSRF protection and exact immutable references, and fail closed; Graph lifecycle application, live-provider acceptance, automated lifecycle scheduling, and general scheduling remain launch work.
