...
- A complete or externally supported general Agent Registry, Loop, Graph, or Execution Queue product. The narrow public CLI/API vertical, authority-bound Loop activation/retirement, bounded Hermes gateway execution, live console inspection, and authenticated Graph compose/publish and typed-run forms exist. Those browser forms are narrowly constrained to exact immutable Agent/Loop resolution, authority derived from a runtime session, same-origin CSRF protection, canonical typed inputs, conflict-detecting idempotency, and fail-closed admission; Agent/Loop/Queue browser mutation, Graph lifecycle application, live-provider acceptance, and general scheduling remain launch work.
...
- [x] `/v1/agents`, `/v1/loops`, `/v1/graphs`, and `/v1/queue` call shared services and return distinct denied, unavailable, conflict/not-found/repair-required, and genuinely empty states where applicable.
- [x] Credential-independent fleet actions and the installed acceptance proof run without downstream credential setup.
