...
- A complete or externally supported general Agent Registry, Loop, Graph, or Execution Queue product. The narrow public CLI/API vertical, authority-bound Loop activation/retirement, bounded Hermes gateway execution, live console inspection, and the two reviewed dashboard Loop publication/lifecycle commands exist, but browser mutation controls for other domains, Graph lifecycle application, live-provider acceptance, and general scheduling remain launch work.
