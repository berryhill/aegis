...
flowchart TB
  ...
  Revocation[Append-only authority revocation] --> Admission[Fresh runtime admission]
  Context --> Admission
...
Canonical domain ownership is bounded rather than represented by one mutable lifecycle aggregate. `core` owns subject, stanza decision, effective authority, mandate, immutable per-session authority context, append-only revocation, session, and provisioning artifact/receipt records. `execution` owns dispatch and runtime-turn state plus the narrow launch/admission contract. `evidence` owns content-addressed runtime artifacts and claim-specific verification receipts. The application service composes those records, and the Hermes adapter receives only an immutable launch contract and fresh admission decision. Evidence cannot select authority or declare a broader domain workflow complete. The former experimental plumbing aggregate, universal validator, POC orchestrator, GraphRun readback, CLI command, and API routes are removed rather than renamed.
