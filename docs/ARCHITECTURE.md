...
flowchart TB
  ...
  AuthorityRepo[Typed authority repository foundation] --> Facts[Hash-linked transition facts]
  ...
  Service --> State[(Charters, plans, approvals, receipts, mandates, authority contexts, revocations, sessions)]
  ...
  Manager -->|close: invalidate inference capability before model teardown| Cleanup[Bounded manager cleanup]
  Cleanup --> Proxy
