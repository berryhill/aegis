...
Each session instantiates one immutable authority context from the canonical mandate before starting a new Hermes process and disposable home with a minimal environment. The context binds the exact session, subject, logical agent, charter revision and digest, runtime descriptor, effective capabilities/toolsets, memory and credential scopes, issuance interval, and canonical digest. It cannot widen or replace the mandate.

Ordinary toolsets use safe mode, which disables inherited project rules, user memories, plugins, and MCP. Aegis passes exactly the approved stanza toolset arguments and selected provider binding, verifies the resolved launch arguments, and records `toolset_verification: launch_arguments`. Runtime adapters receive only a narrow immutable launch contract and must obtain a fresh authoritative admission decision bound to the exact authority-context ID and digest immediately before an effect.
