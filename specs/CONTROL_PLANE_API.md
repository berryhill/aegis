...
The authenticated manager turn route returns authoritative non-model results for generic or polite Agent-registration and credential-create intent. Agent intent names the deterministic `/agents` transaction; credential create reports no mutation because protected no-echo intake is unavailable through this endpoint. It MUST NOT turn that boundary into a generic 503 or forward credential-bearing text to the model. Conversational failures use stable `manager_runtime_unavailable`, `manager_authority_unavailable`, `manager_authority_invalid`, `manager_turn_timeout`, `manager_turn_protocol_error`, and `manager_turn_internal_error` envelopes with matching HTTP status. Clients MUST reject unknown or status-inconsistent categories and render bounded remediation rather than producer/model text.
