...
Aegis authenticates the configured principal
  ...
  -> unloads an external-local model only when this session owns its successful load
...
After authenticated fresh or resumed onboarding reaches `Ready`, bootstrap MUST separately request default-decline approval to create exactly one canonical immutable Agent registration: Agent ID `aegis`, revision 1, source kind `aegis-system`, fleet/source `aegis/builtin/aegis`, runtime `hermes-agent`, target `manager-disposable`, owner `aegis-system`, and accountability bound to the authenticated principal. `agent_id=aegis` MUST be presented as a logical Agent Registry identity, and `runtime_target=manager-disposable` MUST be presented as a disposable runtime contract rather than a named Hermes profile. Exact replay MUST be idempotent; any non-canonical collision or incomplete full-record readback MUST deny. Generic lifecycle/revision paths MUST NOT mutate this record. Registration MUST NOT create `~/.hermes/profiles/aegis`, inspect or inherit normal `~/.hermes`, or grant credential, stanza, mandate, runtime, queue, or model authority. Supported launch MUST be bare `aegis` or `aegis manager`, never `hermes --profile aegis`. Approval, successful creation, and exact-replay output MUST state these identity, runtime, filesystem, and launch boundaries. A store owned by an already-running pre-feature gateway MUST NOT be backfilled by a second writer; any future backfill MUST be gateway-owned and explicitly approved.

After the canonical built-in record is verified, the same fresh or resumed offline bootstrap MUST check the principal-stable local-default import identity before manager activation. An exact existing import MUST be verified from its canonical immutable initial charter/revision plus its current no-capability/no-policy Registry revision without rereading mutable profile evidence or presenting another approval; the current lifecycle MUST be reported accurately. Otherwise bootstrap MAY inspect only the configured principal's canonical default `~/.hermes/config.yaml` through the same owner/type/mode/no-follow, metadata-only boundary used by `/agents import hermes default`. A missing root or marker MUST be non-gating. Unsafe evidence, repository errors, and Agent/source collisions MUST fail closed.

An eligible profile MUST produce two distinct default-decline decisions: a read-only review of the sanitized fingerprint and complete deterministic proposal, then exact registration approval. Review acceptance MUST NOT write. Registration approval MUST regenerate all evidence, require the reviewed revision digest, execute the shared immutable import transaction, and verify complete readback. The newly imported Agent MUST be enabled (Registry eligibility, not runtime readiness or activation), principal-owned/accountable, bound to runtime target `aegis-owned-ephemeral`, and declare no capabilities or policies. Bootstrap MUST NOT copy, read, hash, run, activate, or inherit profile contents, credentials, skills, memories, plugins, MCP state, model binding, or ambient authority. Decline, invalid input, EOF, or interruption MUST perform no import and MUST preserve a later offline Ready offer. After successful confirmation, authoritative Registry list readback MUST contain both the built-in `aegis` Agent and the imported `hermes-default-*` Agent.
...
### 4.5 Platform expertise and authoritative intent routing

Every manager session MUST receive the exact digest-bound `aegis.platform.expertise.v3` projection. Its content MUST cover Agent Registry, Loop, Graph, Queue, evidence/disposition, external authority, and credential/capability separation; manager policy and certification identity MUST change when the projection changes. The model persona MUST be the credential-value-blind conversational component of the built-in Aegis manager, with credential custody represented as one subsystem rather than its complete identity. The projection is explanatory model context and MUST NOT grant authority.

Natural-language registration MAY prepare or recommend but MUST NOT confirm, register, or activate an Agent. Unqualified registration MUST return no-mutation typed-control guidance; only an explicit local-default-profile request may enter that separate non-authorizing import preparation path. Complete manager ensure/update/restart wording MUST return authoritative no-mutation guidance naming typed status, update, and restart controls, and MUST NOT claim that status was checked, installed state is current, or a restart occurred.

Both conversational and degraded terminal headers MUST make `/agents` discoverable and name `/help agents` for exact grammar. Generic and polite Agent-registration and credential-create imperatives MUST be classified before model dispatch. Complete low-ambiguity credential count, list, filtered-search, and exact-reference terminal-only value-read requests MUST use one shared deterministic dispatcher before model dispatch in both in-process and supervised-gateway sessions. Each handled read MUST execute through the authenticated Aegis authority, return an authoritative typed result, and MUST NOT be vetoed, selected, approved, or performed by the model. Educational, quoted, negated, multiline, or incomplete follow-up text MUST NOT manufacture a read operation. Closed anchored forms asking how to install the official Aegis Hermes skills, whether an imported Hermes Agent inherits profile expertise, or whether a registered Agent can be renamed MUST return authoritative deterministic guidance before model dispatch. That guidance MUST identify exact supported installation commands, MUST state that Registry import copies no profile skills, configuration, prompts, memories, plugins, credentials, or model binding, and MUST distinguish immutable canonical Agent IDs from the currently unsupported display-alias operation. It MUST NOT install a skill, rename identity, mutate Registry state, or grant authority. Registration intent MUST return deterministic authoritative `/agents readiness` and prepare/register guidance without claiming mutation. The gateway turn endpoint has no protected no-echo intake and therefore MUST NOT create a credential; it MUST return deterministic no-mutation protected-intake guidance and MUST NOT forward supplied credential-bearing syntax to the model. Runtime unavailable, authority unavailable, authority invalid, timeout, protocol, and internal turn failures MUST remain closed typed categories from producer through HTTP and client remediation; category selection MUST NOT parse arbitrary error/model text.
...
Aegis-authoritative create turn
  <-> authenticated principal and deterministic intent parser
  <-> session-scoped sensitive Aegis state; no model dispatch
...
### 7.4 Protected credential-value admission

The conversational manager is credential-value-blind. A clear authenticated create imperative MAY establish only bounded create metadata and authorization intent. Aegis MUST:

1. require active principal authentication, active mandate, and the built-in secrets-manager stanza;
2. deterministically derive bounded metadata outside the model;
3. strip every recognized inline value from presentation/history, wipe captured bytes, and refuse to select among multiple supplied values;
4. bypass Hermes/model negotiation for the typed create intent;
5. collect the value again only through Aegis-owned protected no-echo intake after the applicable authorization and confirmation; and
6. retain only encrypted authority state and metadata-only audit/receipt state after closure.

The model response MUST NOT authorize the operation. Authorization comes from authenticated deterministic Aegis controls and is limited to the exact typed target presented to the principal.

The deterministic create grammar MUST cover documented natural imperatives, including `store`, `save`, `create`, and `make`. Independently of successful intent mapping, explicit credential-value syntax MUST fail closed before model dispatch: an unrecognized create verb may produce a local retry diagnostic but MUST NOT send or retain the value-bearing input. High-confidence ordinary credential material MUST be blocked; arbitrary-text secret detection remains defense in depth rather than a complete DLP guarantee.

Low-ambiguity authenticated read intents such as credential count, metadata list, and exact-reference value retrieval MUST execute directly through Aegis authority operations without a model round trip or confirmation. Exact-reference retrieval grammar MUST accept both value-leading forms such as `value for credential: "demo"` and reference-leading forms such as `see the demo cred value`; it MUST NOT infer a reference from a pronoun or missing identifier. Count MUST be exact rather than inferred from a bounded list and MUST distinguish total, active, and record-level revoked records. Count/list output and all audit MUST remain metadata-only. Value retrieval MUST require an explicit exact reference, reject missing or revoked records, terminal-escape rendered plaintext, and retain it only in session-scoped presentation state. Terminal scrollback remains outside cleanup. Questions and explanatory discussion MUST NOT become mutation proposals. If no inline value is supplied, the authenticated imperative proceeds directly to protected no-echo intake without a separate mutation approval. Create MUST be atomic insert-only; an existing reference MUST fail without replacement, and rotation/replacement retains confirmation. Authority passphrases, KEKs, provider credentials, and values from other sessions remain unauthorized for model admission.

Protected terminal intake MUST accept a terminal-framed bracketed multiline paste as one bounded no-echo value and MUST require an independently framed matching confirmation. The submit delimiter terminating each framed paste MUST be consumed before the ordinary composer resumes. Confirmation mismatch or protected-intake failure MUST discard queued protected terminal input so residual lines cannot become a later manager/model turn. Credential-bearing ordinary-composer input MUST be stripped and rerouted to protected intake when it is a recognized deterministic create; otherwise high-confidence credential material MUST be blocked before Hermes. Terminals without bracketed-paste framing MAY remain line-only when an explicit protected-stdin alternative is available.
...
- reject every policy finding and high-confidence secret detection; no conversational plaintext-secret allow mode exists;
- require recognized create and exact-value operations to have been consumed by the authoritative Aegis route before this boundary; arbitrary-text detection is defense in depth rather than complete DLP;
...
`external-local` is an operator-managed endpoint. It MUST be opt-in and visibly described as a weaker lifecycle/attestation boundary. Before Load, Aegis MUST record exact name+digest running presence. It MUST preserve a pre-existing runner, own cleanup only when the runner was absent and Load unambiguously succeeded, and infer no ownership or unload after a failed/ambiguous Load. Unload is a name-only Ollama mutation guarded by exact inventory checks, not atomic digest-bound mutation; shared-daemon same-name TOCTOU remains residual.
...
The wizard MUST prefer the strongest custody mode that is actually deployable for the complete selected lifecycle. Bare onboarding that proceeds to the built-in automatic rootless gateway MUST default to an owner-only host KEK and MUST disclose same-account readability, exact paths, backup-separation requirements, and the local-root limitation before creation. Passphrase custody MUST remain available only as an advanced interactive route; after verified unlock, bare `aegis` and `aegis manager` MUST retain authority in-process and enter the local manager without attempting automatic user-service activation: it collects and confirms the passphrase through fresh bounded protected pinentry requests after plan authorization, falls back only before protected interaction to terminal-backed no-echo intake, persists only an authenticated encrypted envelope, and verifies authority before continuing. Cancellation or post-interaction helper failure MUST NOT switch input surfaces. Systemd custody MUST be offered only when its external delivery and unit-integration prerequisites are explicit; an ordinary shell MUST NOT be told that it can emulate `CREDENTIALS_DIRECTORY`. Deterministic user-service preview MUST remain available for inspection, reset, and exact unit purge regardless of configured custody. Automatic user-service application MUST reject passphrase and externally integrated systemd custody before service-manager mutation until it can prove an exact noninteractive protected-input delivery path.
...
Each live case execution MUST use `manager.hermes.turn_timeout`. A schema-valid response whose sole failure is missing required conversational content MUST receive a fresh execution, with at most three total attempts. A bounded segment MUST abort on the first timeout, cancellation, authority expiry, protocol/transport error, invalid response, other failed requirement, or exhausted conversational-content check; it MUST report the exact case plus a stable metadata-safe reason and MUST NOT publish a partial certification. Because Hermes turn events lack prompt correlation, a session interrupted after prompt submission MUST be destroyed rather than reused. Fresh principal authority expiry MUST bound each complete segment, including setup, cleanup, audit, and checkpoint/final publication, and fail closed with `manager_session_expired`.

Certification MAY resume from an owner-only, strictly validated, exact ordered pass-prefix checkpoint under a newly authenticated principal and newly verified model/runtime tuple. The checkpoint is not a certification, readiness artifact, authority, or permission for a model to choose cases. It MUST bind the exact principal, candidate, model digest and quantization, Hermes/Ollama versions, context length, instruction/schema/conformance identity, and corpus digest; stale, mismatched, malformed, unsafe, duplicate, reordered, or failed-case records deny. A segment never splits a case, and no failed or interrupted case is credited. Final publication requires every exact case once, current principal authority, successful cleanup/audit, full `Certification.Validate`, and readback of the saved certification. The 15-minute principal authentication maximum remains unchanged.
...
A Hermes child exit, proxy failure, protocol violation, or model drift MUST stop model turns, close the failed conversational runtime, and move the still-authenticated manager session to typed `manager_runtime_failed` degraded mode. The failure MUST NOT revoke the principal identity, stanza, mandate, or deterministic manager capability; `/status`, `/help agents`, `/agents readiness`, and `/agents list` remain gateway-owned and available while their own authority prerequisites remain valid. Ordinary conversational turns fail closed with no automatic provider or model fallback. If failed-runtime cleanup returns an error or times out, the gateway retains sticky occupancy evidence and denies replacement conversational admission until process restart or explicit verified reconciliation; deterministic controls in the existing authenticated session remain available. Session expiry, revocation, gateway context cancellation, or an authority failure that invalidates the manager capability still enters full cleanup and revokes the session. A gateway manager mandate MUST expire at the authenticated principal boundary rather than the independent browser-console SessionTTL. That deadline MUST bound deterministic commands, authoritative turns, and model turns; normal expiry MUST wake idle input, cancel in-flight work, tear down the disposable runtime, render fresh-session guidance, and MUST NOT silently renew or resume authority.
...
6. for an Aegis-owned external-local load only, require exact unambiguous inventory and request name-based model unload;
7. verify exact absence under a bounded deadline;
