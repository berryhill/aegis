...
A Hermes child exit, proxy failure, protocol violation, or model drift MUST stop model turns, close the failed conversational runtime, and move the still-authenticated manager session to typed `manager_runtime_failed` degraded mode. The failure MUST NOT revoke the principal identity, stanza, mandate, or deterministic manager capability; `/status`, `/help agents`, `/agents readiness`, and `/agents list` remain gateway-owned and available while their own authority prerequisites remain valid. Ordinary conversational turns fail closed with no automatic provider or model fallback. If failed-runtime cleanup returns an error or times out, the gateway retains sticky occupancy evidence and denies replacement conversational admission until process restart or explicit verified reconciliation; deterministic controls in the existing authenticated session remain available. Session expiry, revocation, gateway context cancellation, or an authority failure that invalidates the manager capability still enters full cleanup and revokes the session.
