...
After authenticated fresh or resumed onboarding reaches `Ready`, bootstrap MUST separately request default-decline approval to create exactly one canonical immutable Agent registration: Agent ID `aegis`, revision 1, source kind `aegis-system`, fleet/source `aegis/builtin/aegis`, runtime `hermes-agent`, target `manager-disposable`, owner `aegis-system`, and accountability bound to the authenticated principal. Exact replay MUST be idempotent; any non-canonical collision or incomplete full-record readback MUST deny. Generic lifecycle/revision paths MUST NOT mutate this record. Registration MUST NOT create a persistent Hermes profile, inspect or inherit normal `~/.hermes`, or grant credential, stanza, mandate, runtime, queue, or model authority. A store owned by an already-running pre-feature gateway MUST NOT be backfilled by a second writer; any future backfill MUST be gateway-owned and explicitly approved.
