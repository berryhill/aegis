...
Live certification MUST place the configured `manager.hermes.turn_timeout` around every gateway turn. An interactive bootstrap approval MAY authorize the finite, exact corpus to continue across bounded segments in one command; it is not authentication and may not authorize any unrelated operation. Each bounded certification segment MUST use a freshly authenticated principal, including when the preceding segment ran in the same command; that subject's expiry bounds setup, every complete case in that segment, cleanup, audit, and checkpoint or final publication. The principal authentication TTL remains capped at 15 minutes and MUST NOT be lengthened to accommodate a slow model. An exact ordered pass prefix MAY be checkpointed separately under owner-only Aegis state after successful cleanup and audit. Such a checkpoint is nonauthorizing: it is never a certification, model identity, runtime grant, or manager-readiness artifact. Every resume reauthenticates and rechecks the exact principal, model digest/details, Hermes and Ollama versions, context length, instruction/schema/corpus identity, and checkpoint custody before running the next whole cases in a fresh disposable Hermes/proxy session. A segment MUST NOT split a case or reuse a session after interruption. Only an exact all-case pass with fresh authority, successful cleanup and audit MAY publish the existing full certification, followed by exact readback.

A schema-valid response whose sole failure is missing required conversational content MUST be retried in a fresh execution, with at most three total attempts. The first timeout, cancellation, expiry, protocol/transport error, invalid response, other semantic failure, or exhausted conversational-content check MUST abort the current segment and cleanup the disposable Hermes/proxy/model resources; the interrupted case MUST NOT advance a checkpoint. Since late Hermes events are not prompt-correlated, an interrupted session MUST NOT be reused. Failure output MUST include the exact case and a stable metadata-safe reason and MUST NOT publish a partial certification. An expired, mismatched, malformed, unsafe, stale, duplicate, reordered, or failed-case checkpoint MUST fail closed rather than becoming readiness.
