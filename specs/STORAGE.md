...
| Fleet-control definitions | Agent, Loop, and Graph identities/revisions; validation records; Graph run snapshots | Badger `github.com/dgraph-io/badger/v4` `v4.9.5`, schema `fleet-v1` | Linux/amd64, ext4 | one Aegis process holding the exclusive fleet writer lock | shared `state/persistence/fleet-v1`; directory `0700`, files `0600`, `SyncWrites=true`, `DetectConflicts=true`; strict canonical decode, immutable create/conflict denial, digest verification on read; clean-or-verified-recovered open; fixed 256 MiB free reserve | domain owners through a future narrow fleet adapter |
| Fleet-control lifecycle | submissions/rejections, queue transitions, claims/leases, Graph runs, Loop executions, attempts, retries, cancellations, expiry, disposition | Badger `github.com/dgraph-io/badger/v4` `v4.9.5`, schema `fleet-v1` | Linux/amd64, ext4 | one Aegis process holding the exclusive fleet writer lock | same shared `state/persistence/fleet-v1`; directory `0700`, files `0600`, `SyncWrites=true`, `DetectConflicts=true`; atomic submission disposition and single-winner claim/terminal transition; durable attempt history and lease recovery; clean-or-verified-recovered open; fixed 256 MiB free reserve | `internal/queue` and `internal/execution` through the same future narrow fleet adapter |
