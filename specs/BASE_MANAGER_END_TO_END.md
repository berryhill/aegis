...
- After exact built-in readback and before manager activation, fresh/resumed offline bootstrap MUST verify an exact prior local-default import or inspect only the configured principal's canonical default profile through the metadata-only no-follow boundary. A missing profile is non-gating; unsafe evidence and collisions deny.
- An eligible profile MUST receive separate default-decline read-only review and exact-digest registration decisions. Review performs no write. Confirmation regenerates evidence and creates only one immutable disabled `hermes-default-*` Agent with no capabilities or policies; it MUST NOT copy, run, activate, or inherit profile contents or authority.
- Successful confirmation MUST perform authoritative list readback containing canonical built-in `aegis` plus the imported Agent. A later invocation MUST verify the canonical immutable import charter/initial revision and the current no-capability/no-policy revision without rereading profile evidence or repeating approval; current lifecycle MUST be reported accurately.
