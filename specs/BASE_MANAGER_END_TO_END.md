...
- An eligible profile MUST receive separate default-decline read-only review and exact-digest registration decisions. Review performs no write. Confirmation regenerates evidence and creates only one immutable enabled `hermes-default-*` Agent with no capabilities or policies; it MUST NOT copy, run, activate, or inherit profile contents or authority.
