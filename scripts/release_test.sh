...
p = Path("CHANGELOG.md")
t = p.read_text()
# Mirror scripts/release.sh prepare_changelog: move the existing Unreleased
# entries under the new release heading, leaving an empty Unreleased section.
marker = "## Unreleased"
if t.count(marker) != 1:
...
p.write_text(before + unreleased_block + body_after_next)
'
