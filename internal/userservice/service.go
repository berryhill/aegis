...
func validateAutomaticServiceCustody(configPath string) error {
	inspection := config.Inspect(configPath)
	if inspection.State != config.StateValid || !inspection.FilePresent {
		return errors.New("user service requires one secure file-backed valid configuration")
	}
	if authority := inspection.Config.Credentials.Authority; authority.Database != "" && authority.Custody != "host-file" {
		return fmt.Errorf("automatic user service requires gateway-compatible host-file credential custody; configured custody %q requires explicit external service integration", authority.Custody)
	}
	return nil
}
...
func Apply(ctx context.Context, plan Plan, runner Runner, timeout time.Duration) error {
	...
	if err = validateAutomaticServiceCustody(plan.ConfigPath); err != nil {
		return err
	}
