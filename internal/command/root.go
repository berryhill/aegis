...
func NewRoot(deps Dependencies) *cobra.Command {
	...
	root.RunE = func(cmd *cobra.Command, _ []string) error {
		...
		if requiresGateway(deps.Profile) {
			...
			// Bare startup presents the authenticated browser-console boundary and
			// returns. Entering the terminal manager remains an explicit action so
			// incomplete model onboarding cannot masquerade as usable conversation.
			return launchConsole(cmd, o, deps.OpenBrowser)
		}
