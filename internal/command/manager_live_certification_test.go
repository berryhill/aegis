...
func TestLiveManagerCertification(t *testing.T) {
	...
	cert, runErr := managerdomain.RunCertificationWithOptions(ctx, probe, candidate, candidate.OllamaName, digest,
		model.Details.QuantizationLevel, "installed-hermes-probe", version, cfg.Hermes.ContextLength, time.Now().UTC(), managerdomain.CertificationOptions{})
	if runErr != nil {
	...
	if err := cert.Validate(); err != nil {
		t.Fatal("complete corpus invalid:", err)
	}
	if err := cleanup.close(); err != nil {
		t.Fatal("cleanup failed before pass:", err)
	}
	t.Logf("live corpus passed: cases=%d elapsed=%s; certification_not_saved=true", len(cert.Results), time.Since(deadline.Add(-15*time.Minute)).Round(time.Second))
}
