...
- Made the installed console browser proof timing-resilient for slow continuous-integration runners. The proof now extends the demo `session_ttl` and `auth_ttl` to ten minutes (from one and two minutes respectively) so the entire interactive native-form rotation flow remains authenticated for the full browser proof, and the browser-proof `wait_for` reports per-step elapsed time, the configured timeout, and the captured document-request count in its failure message so future CI failures self-diagnose whether the page re-rendered, the session expired, or the browser simply lagged the predicate. Unit tests cover the new diagnostic output.
