...
- Corrected Loop canvas placement with external-stylesheet selectors generated from bounded application-owned integers through `loop.MaxSteps`, explicit row and column placement, fixed implicit tracks instead of the eight-column template limit, and fixed border-box node dimensions. Source regression coverage checks supported selectors, reversed-declaration branching/joining, projected endpoints, and exact nonzero node/transition coverage with missing-edge mutation controls. Browser-computed geometry and before/after visual acceptance remain separate verification obligations. The installed geometry fixtures now include an exclusive gate and explicit join, with production-validator regression coverage for both complete fixture revisions and denial of the original missing-gate shape; browser acceptance additionally requires actual keyboard-pan and mouse-drag displacement. Narrow Loop pages now retain authenticated status and logout controls within the viewport rather than widening a requested 390px mobile layout; geometry failures report the exact fixture/phase and actual/expected revision and width without dumping page content.

- Fixed first-claim eligibility and completion audit-mandate binding for workspace-origin Queue work. Claimability reloads the exact immutable runtime binding rather than treating its transition as a retry; completion uses that binding's mandate without rewriting submission provenance. Real-store HTTP regressions cover live-lease denial, expiry-backed reclaim, preserved causal identities and authoritative runtime failure. Synthetic runtime fixtures are not supported-Hermes or installed-agent success evidence.

- Reconciled the distributed credential skill with the delivered gateway-owned protected-intake protocol. Bundled self-contained operator/recovery guidance and synthetic metadata contracts; an installed-archive regression validates them with the actual public decoder and denies missing identities and invented authority/value fields. This is installed-artifact contract evidence, not supported-Hermes agent behavior or public release acceptance.
...
- Corrected Registry skill registration readback to the public `{agent, created}` envelope and preserved identical Agent content on replay. All reduced Registry interpretation fixtures now explicitly identify themselves as explanatory projections, not complete wire responses; regression tests enforce that evidence boundary.

- Corrected projection reconciliation operation metadata to `unavailable` while retaining the shipped advisory skill. Structural evaluation rejects routes to non-shipped operations; the normal projection request fixture now expects an unavailable denial rather than a fictional executable route.

- Corrected skill evaluation JSON to report structural fixture validity rather than unqualified passes, with behavioral/service/runtime execution explicitly not run. Consumers of `passed` must migrate to `structurally_valid` and the explicit evidence class. Added library and CLI output regressions. Queue guidance now documents the controller-only `bind-runtime` CLI (and missing online adapter) and refuses unsupported non-reclaimed retry without weakening persistence admission.


- Defaulted omitted new-Agent registration lifecycle to `enabled` before preview and exact-digest approval, including new explicitly approved default-Hermes-profile imports. Registry eligibility grants no runtime readiness, activation, workspace delegation, credentials, or execution authority. Explicit disabled/retired inputs and existing registrations remain unchanged; malformed explicit lifecycle values deny. Manager import review now renders the proposal lifecycle rather than stale disabled-default wording.
- Strengthened installed registration verification to omit lifecycle and check enabled review, collection, detail, refresh and authenticated server-restart readback before provisioning, with exact canonical revision equality and a no-gateway-invocation assertion.
- Corrected the installed-console proof to open the Queue node inspector and its Back to queue definition control before asserting visible authoritative evidence. Retained route focus, evidence/receipt/disposition, responsive layout and inspector-focus checks instead of racing the initial canvas-first collapse.
- Added opt-in checksum/revision-bound installed-CLI PTY coverage for protected credential creation, using real authenticated Unix API and encrypted custody with isolated read-only service-manager fixtures. Strengthened metadata approval, reopened value equality and plaintext-canary checks. This does not claim live-model or systemd deployment acceptance.
- Reconciled gateway credential-create launch guidance across README, quickstart, security policy, threat model, architecture diagram and contributor coverage. Distinguished the gateway's fresh metadata approval from legacy in-process immediate creation; retained unsupported-client denial and uncertain-persistence limits.
- Added guided protected credential creation to the gateway-backed Linux terminal. Conversational requests now initiate a session-bound metadata review and explicit approval, followed by separate no-echo value/confirmation intake and canonical encrypted creation. Replay, expiry, session substitution, browser/unsupported clients and unapproved writes deny. Uncertain persistence is never automatically retried; unsynchronized input failure stops the conversation. Added real Unix-gateway/PTY-to-bbolt tests for typed and multiline intake, cancellation and metadata persistence across reopen.


- Reconstructed the Queue execution detail as the accepted pinned-control-flow workspace. The dedicated `#/queue/:execution-id` route now rebuilds the exact pinned Graph/Loop revision from the queue snapshot binding (never substituting current catalogue topology), projects authoritative runtime state onto pinned definition nodes and transitions (current control, attempt/iteration, taken/not-taken transitions, cycle bounds, terminal eligibility, failure location), and renders six supporting tabs (Inputs and outputs, Timeline, Authority and runtime, Evidence, Admission, Submission snapshot) with the accessible textual equivalent under strict CSP. A single contextually eligible reviewed operation replaces the pre-change vertical controls. Artifacts, passing receipts, process exit, and model narration no longer upgrade execution state. Catalog drift and reconstruction warnings surface when the pinned revision is absent. Historical executions remain reconstructable after definitions advance.
- Rebuilt the Loop detail as the accepted interactive control-flow workspace. The dedicated `#/loops/:loop-id` route now keeps the human title/stable ID/version hierarchy, renders description, lifecycle, stored validation, control-flow/input/evidence/iteration metrics, and exact Graph-child execution explanation from authoritative data only, adds bounded pan/zoom/fit with a CSS-grid stage (no inline styles; `style-src 'self'` CSP preserved), and exposes an explicit cycle band with members, maximum iterations, exit condition, and exhaustion destination. Selecting a semantic node opens a right-side contextual inspector with identity, expected output, capabilities, timeout, mappings, evidence claims, failure behavior, and incoming/outgoing transitions; the lower-page anchor list is gone and the supporting Required inputs / Evidence and outcomes sections are now two real tabs. Lifecycle controls remain digest-bound and authoritative. Cyclic, branching, invalid, terminal-only, maximum-size, and historical Loop fixtures remain inspectable and truthful; CSP/no-script fallback still provides an accessible textual equivalent; Loop definitions never become independently executable and action handoff stays Graph-mediated. README and SECURITY were updated to mention the Loop detail workspace and the shared presentation script's Loop pan/zoom/fit role.
...
- Added fresh/resumed offline bootstrap discovery and explicit import of the configured principal's existing default Hermes profile. After the sealed built-in `aegis` record is verified, Aegis now verifies an exact prior import or safely inspects canonical `~/.hermes/config.yaml`, presents separate default-decline read-only review and exact-registration decisions, and creates one immutable enabled `hermes-default-*` Agent only after digest-bound confirmation and full readback. Missing profile evidence remains non-gating; unsafe evidence and collisions fail closed; no profile contents, credentials, skills, memories, plugins, MCP state, model binding, capabilities, policies, activation, or ambient authority are imported.

- Added the official advisory `aegis-deployment-projection` Hermes skill for read-first review of selective signed per-deployment projection inputs, generation and digest comparison, reconciliation, staging, drift, acknowledgment, revocation, interruption recovery, and monotonic rollback semantics. The current public CLI/API has no typed deployment-projection mutation or authoritative readback surface, so the skill reports those operations unavailable rather than simulating them; externally authenticated identity, exact approval, signing, secret selection, activation, rollback admission, and authoritative audit remain fail-closed Aegis responsibilities outside the model.
- Added the official advisory `aegis-operator-lifecycle-diagnostics` Hermes skill for read-first interpretation of installed version/provenance, Aegis deployment roots, gateway plan/activity, direct stable-release updates, migration, bounded reset, interruption recovery, and rollback availability. It routes only to shipped typed Aegis surfaces, distinguishes command-specific authentication and confirmation boundaries, separates `gateway status` activity from authenticated readiness, treats package-manager ownership as operator-supplied context, and never turns fixtures, process activity, model narration, or mutable release references into authority or completion evidence.
- Added authenticated fresh/resumed bootstrap registration of the canonical immutable built-in `aegis` Agent with Aegis ownership, principal accountability, explicit default-decline approval, exact idempotent readback, and collision denial. `agent_id=aegis` is a logical Agent Registry identity and `runtime_target=manager-disposable` is its disposable runtime contract, not a named Hermes profile: registration does not create `~/.hermes/profiles/aegis`, and supported launch is bare `aegis` or `aegis manager`, not `hermes --profile aegis`. The record grants no credential or model authority. Gateways already running from a pre-feature process are deliberately not silently backfilled while they own the stores; an explicitly approved gateway-owned backfill remains future work.
- Added the digest-bound `aegis.platform.expertise.v3` manager projection for Registry/Loop/Graph/Queue/evidence/authority/credential boundaries, discoverable `/agents` controls, authoritative pre-model routing for generic or polite Agent-registration and credential-create intents, deterministic protected-intake guidance where gateway turns cannot create credentials, and typed runtime/authority/timeout/protocol/internal turn failures.
...
- Added the official advisory `aegis-manager-onboarding` Hermes skill for artifact-derived initialization and resumption, exact local-model discovery/configuration/certification, authenticated gateway and console handoff, manager launch, interruption recovery, and bounded cleanup through shipped typed Aegis commands. It requires exactly one externally authenticated trust stanza per operational session; denies zero or ambiguous matches; requires a new mandate and clean runtime session for stanza or material-authority changes; and never lets prompt/model narration choose authority or create, replace, or attest authoritative Aegis audit events.
...
- Corrected Credentials-only narrow header spacing so password rotation and sign-out remain visible at 390px without hiding authentication status. Extracted attempt-8 binary proof exposed the overflow; the source browser regression covers the repair. Successor exact-candidate installed acceptance remains required.

- Scoped reset's final decline, EOF, cancellation, and non-terminal result to unapplied destructive deletion. A separately approved gateway preparation remains in effect; the final result no longer claims the overall operation was unchanged or performed no writes.

- Corrected the Credentials workspace (#234): semantic Reference/Kind/Status/Version inventory (Version hides at inventory widths up to 540px or viewport widths up to 760px; Kind hides at inventory widths up to 420px), independently scrolling 42/58 selected desktop panes, and one visible pane at widths up to 900px. Detail retains reviewed-operation entry points, concise metadata, and immutable version history instead of vault/debug dossiers. Fragment selection navigates to the authenticated server-backed record query; per-history-entry presentation snapshots restore collection filters, selected-row focus, and inventory scroll. Native query-bearing links remain the no-script fallback; browser state grants no authority.

- Established explicit authenticated console detail contracts for all five domains: Agent inline, Graph/Loop/Queue replacement pages, and Credentials master/detail. Canonical `#/domain/:id` fragments make record identity visible while bounded query input remains only a server-authoritative lookup request. Typed domain projections now carry topology, authority, evidence, and relationships instead of generic inspector strings. Canonical filters, pagination, and selection are retained, with a same-origin presentation-only enhancement restoring selected-record focus and bounded scroll position; native deep links, refresh, history, and Back links remain complete without scripts or storage, and browser state cannot select identity, stanza, mandate, credential material, authority, or outcome.

- Extended the authenticated manager's deterministic self-knowledge path: conversational framing such as `hey, let's register an agent` now returns authoritative no-mutation guidance without silently choosing a Hermes profile, while complete questions about installing Aegis Hermes skills, imported-Agent expertise, and Agent rename semantics return authoritative product guidance without model veto. Gateway manager mandates now use the principal-authentication boundary instead of the shorter browser-console TTL, and known normal expiry exits cleanly with fresh-session guidance instead of a generic HTTP 401 failure.
- Routed low-ambiguity conversational Agent registration to deterministic `/agents readiness|prepare|register` guidance, reserved non-authorizing local-default import preparation for requests that explicitly name that profile, and routed complete Registry count/list/show requests through authenticated Aegis authority instead of the untrusted model. Rich manager input now consumes complete navigation sequences at empty/history boundaries without exiting or clearing drafts, deletes UTF-8 runes safely on Backspace/Ctrl-H, and classifies actual interruption explicitly.
- Unified authenticated credential count, list, filtered-search, and exact-reference value reads behind one deterministic Aegis authority dispatcher used by both the in-process manager and supervised gateway. Complete low-ambiguity requests now return `AEGIS / authoritative` without model dispatch or model veto, value results are explicitly sensitivity-typed on the authenticated local manager transport and never become model input, and educational, quoted, negated, multiline, or incomplete follow-up text cannot manufacture an operation.
- Removed conversational trusted-local plaintext admission: recognized inline create values are now all stripped from presentation/history, captured bytes are wiped, multiple values are never selected, and the credential is collected again only through protected no-echo intake. High-confidence credential material remains blocked before Hermes; arbitrary-text detection is explicitly defense in depth rather than complete DLP.
...
- Deny multiple authenticated authorized stanza matches before requested-stanza narrowing; a request can no longer disambiguate overlapping authority. Regression coverage retains zero-match, unauthorized-request, and non-unioned single-stanza behavior.
- Bind source-build Git subprocesses to the verified checkout and verify Go's embedded revision, commit time, and dirty flag before publishing a binary. The no-key demo and installed verifier share this builder, including nested linked-worktree provenance regression; clean release-source guards remain unchanged.
- Gave the race-instrumented fake-pinentry subprocess a bounded ten-second test budget, preserving the production two-minute limit and explicit short-deadline tests. Strengthened post-interaction denial assertions to require the expected typed failure and no returned protected value. Reconciled launch documentation with principal-password console authentication, expertise v3, and default host-file custody; regenerated the real no-key development recording without claiming model or installed-candidate success.
...
- Fixed the installed-console real-browser proof remaining on Chrome's persistent `about:blank` document when host-installed extensions were initialized in the fresh proof profile. The proof now starts an incognito headless context, explicitly navigates the selected page target to the exact expected console origin/path, denies navigation errors, and retains the existing fail-closed timeout if the expected page never appears.
- Fixed development `aegis reset` against a stopped operational-authority generation left by a permissive umask or interrupted close. Aegis now holds the exclusive authority maintenance lease from preview through apply; preview narrowly recognizes exact owned single-link Badger files under secure ancestors without writing, and only after confirmation does Aegis tighten the leased generation to `0700`/`0600` through descriptor-anchored no-follow traversal before unchanged strict revalidation and deletion. It preserves `DIRTY`/`CLEAN` evidence, never authorizes the generation for reads, and denies live writers, unknown objects, links, ownership drift, and generation substitution.
- Fixed `aegis reset` under permissive ambient umasks: fleet Badger clean close now syncs and closes the engine, normalizes every Aegis-owned directory to `0700` and regular file to `0600` through descriptor-anchored no-follow traversal, and only then publishes `CLEAN`. Fleet open durably removes stale `CLEAN` evidence and rejects conflicting lifecycle markers. Strict reset validation remains unchanged and still rejects unknown or group/other-writable artifacts.
- Fixed Hermes child and conversational turn/protocol failures revoking or terminating the authenticated manager capability. A failed conversational runtime is now closed and detached while the same authenticated principal, stanza, mandate, and deterministic manager session continue in typed `manager_runtime_failed` degraded mode; the terminal remains open for deterministic controls, incomplete runtime cleanup leaves sticky replacement-admission denial, and model turns remain unavailable with no fallback.
- Fixed configured credential authority startup being silently represented as absent in the separately started gateway. `serve` now fails closed with `manager_credential_authority_unavailable` when its configured custody or repository cannot be opened, readiness derives from the actually configured/open authority rather than an unrelated model reason, and all opened credential/fleet/authority resources close on downstream startup failures.
- Made bare onboarding's recommended custody compatible with the separately started rootless gateway: it now explicitly recommends the owner-only host key and discloses same-account readability. Automatic user-service planning rejects passphrase and externally integrated systemd custody instead of installing a unit that cannot receive the required protected input.
- Fixed development reset of an existing passphrase-custody installation: deterministic user-service preview remains available for exact unit inspection and purge, while incompatible custody is rejected only when automatic service application would begin.
- Fixed live certification cleanup against the supported Ollama `/api/ps` contract: strict decoding now admits the documented `expires_at`, `size_vram`, and top-level `context_length` process metadata while retaining exact model/digest cleanup checks. Certification cleanup is now idempotent and must complete before the certification artifact is published.
- Fixed ready passphrase-custody installations failing immediately after approving manager launch. Bare `aegis` and `aegis manager` now retain the unlocked authority in-process and enter the authenticated local manager directly; host-file custody continues through the supervised gateway.
- Made bare and incomplete `/agents` grammar recoverable in the same authenticated terminal with closed registry-owned usage; unknown caller text remains redacted from responses and logs.
- Made typed slash-command parse failures recoverable across the authenticated manager gateway instead of terminating the terminal, and made manager-session close server-owned and client-visible: runtime cleanup and audit failures are bounded, joined, and returned. External-local model cleanup now unloads only an Aegis-owned single runner matching the configured name and digest after exact inventory checks, preserving pre-existing shared or unknown runners without mutation; certification now surfaces joined reverse-order cleanup failures instead of discarding them.
...
- Fixed ordinary manager requests for local Hermes profile visibility and default-profile import from reaching an incapable model: Aegis now intercepts the closed intents, returns a typed authoritative owner-verified sanitized profile inventory, prepares a deterministic non-authorizing import proposal with principal-stable Agent/fleet/source identities and a metadata-bound revision digest from a non-empty `config.yaml` marker no larger than 1 MiB without reading or hashing its bytes, and requires exact revision-digest confirmation before persisting and rereading one immutable enabled Agent revision. Exact replay is idempotent; no profile contents, credentials, capabilities, runtime authority, activation, provisioning, or mandate are imported.
...
- Made fresh successful bare onboarding finish in a usable agent-control session: after exact gateway installation and authenticated readiness, Aegis enters the authenticated agent terminal directly in the same process without opening the browser console. The manager now exposes closed `/agents readiness`, `list`, `show`, `prepare`, and exact-digest-confirmed `register` operations over the shared Agent Registry service; empty Registry readback is valid, unavailable authority fails closed, preparation is non-authorizing, inputs are bounded regular files, and registration verifies exact post-write readback.
- Removed the passwordless browser bootstrap, exchange, and automatic handoff paths (#161). Explicit `aegis console` exposes only the clean `/console` URL and `principal_password` requirement, while bare startup enters the authenticated agent terminal directly; every new browser session requires the enrolled principal password, and the former Unix bootstrap route returns not found without issuing cookies. Existing exact-origin, host-only `HttpOnly`/`SameSite=Strict` cookie, expiry, logout, CSRF, no-store, throttling, rotation, and invalidation controls remain fail-closed.
...
- Made bare development and production `aegis` enforce the same authenticated gateway install/readiness path before entering the agent terminal directly instead of allowing a checkout-built binary to bypass supervision. Renamed the canonical lifecycle surface to `aegis gateway preview|install|status|start|stop|restart|uninstall`; `aegis service` remains an exact compatibility alias, while explicit `aegis manager` remains the direct authenticated terminal-manager surface.
- Made bare production `aegis` require both ready operational authority and complete manager-model certification before supervised gateway startup. Missing or invalid certification resumes manager onboarding before gateway reconciliation, authenticated readiness, and direct agent-terminal entry.
