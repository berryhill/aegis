...
- Fixed the ready Agent Registry primary action so it is a native same-page link to an always-visible, typed charter-import review containing explicit `aegis charter validate` and `aegis charter import` CLI previews. Denied or unavailable actions remain disabled with their server-owned reason, and the browser remains review-only with no charter mutation or authority input.
