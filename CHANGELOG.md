...
- Exposed the principal-only participant-to-verification proof through `aegis plumbing poc`, `aegis plumbing show GRAPH_RUN_ID`, `POST /v1/plumbing/poc`, and `GET /v1/graph-runs/:id`. The proof requires explicit acknowledgement of its synthetic non-restrictive, non-production stanza; installed CLI/API readback reconstructs and validates one unique terminal aggregate, separately persisted artifact and verification evidence, delivery/disposition bindings, and authoritative audit chain. Missing, corrupt, ambiguous, non-terminal, mismatched, or unverified state fails closed, and a hermetic installed-binary test proves CLI/API parity plus disposable Hermes-home removal.
- Added the end-to-end POC orchestration service that accepts only an already-authenticated, active exactly-one-stanza Hermes authority context, invokes exactly one bounded child Attempt turn, persists runtime output by content digest, obtains evidence from a distinct Aegis verifier through a fresh blob read, and records a terminal causal aggregate plus authoritative audit event. Missing, ambiguous, expired, revoked, tampered, wrong-runtime, or mismatched-parent authority denies before Hermes; the explicitly non-production CLI/API proof surface described above is its only public entry point.
- Added a focused Hermes runtime-adapter operation for exactly one Attempt turn under one validated authority context, with caller cancellation, authority-expiry and explicit duration deadlines, hard input/output ceilings, disposable runtime state, exact parent-dispatch/runtime/stanza/credential checks, and Aegis-owned lifecycle provenance. The operation remains an internal surface; no CLI, API, provisioning, or activation behavior changed.
...
- Defined the participant-centric plumbing aggregate and fail-closed Go validator for the authenticated causal chain from ingress and exact trust-stanza selection through immutable authority, attempts, typed work, evidence, delivery, and terminal disposition. Normal operational sessions still use existing source records; the non-production proof surface above is the first application-service adoption.
- Added narrow state-store primitives for atomic create-only JSON records and exact-byte content-addressed blobs. Records reject replacement, traversal, and symlinked paths; blobs use canonical `sha256:` references, verify existing and read content before acceptance, and fail closed on malformed references or detected corruption. The plumbing POC now uses these primitives for separately persisted aggregate, artifact, and verification-evidence records.
