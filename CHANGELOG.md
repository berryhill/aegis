...
- Made the installed console browser proof deterministic on slow continuous-integration runners. The proof now extends the demo `session_ttl` and `auth_ttl` to ten minutes, verifies browser focus and retained input before submission, and uses native mouse input—not a synthesized mobile touch compatibility event—to open the password dialog under the 390px viewport. An immediate authenticated-state assertion and action-bounded, secret-free authentication-request diagnostics distinguish pre-action session loss from an exact login, logout, session, or password transition.
