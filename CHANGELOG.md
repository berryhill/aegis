...
- Made bare production `aegis` treat ready operational authority—not optional manager-model certification—as the admission boundary for supervised control-plane startup. Missing or invalid certification no longer diverts production startup back into manager bootstrap before service reconciliation, authenticated readiness, and the browser-console handoff; development bare startup and explicit `aegis manager` retain the authenticated degraded-manager path.
- Made bare interactive production `aegis` complete the browser-console handoff in one invocation. It now validates and starts an exact installed user service when needed, requires authenticated audit-current readiness after both existing-service and first-install paths, obtains a single-use bootstrap, and opens the configured `/console` URL through an argument-safe platform launcher. Bootstrap denial opens nothing; browser-launch failure returns the manual URL and bootstrap for an explicit fallback.
- Made `aegis service start`, `stop`, and `restart` validate the exact current Aegis-owned user unit before invoking `systemctl`; an absent unit now returns the stable `service_not_installed` error with the exact `aegis service install` remediation and performs no installation or service-state mutation.
