...
- Extended the authenticated manager's deterministic self-knowledge path: conversational framing such as `hey, let's register an agent` now reaches owner-verified default-profile preparation, while complete questions about installing Aegis Hermes skills, imported-Agent expertise, and Agent rename semantics return authoritative product guidance without model veto. Gateway manager mandates now use the principal-authentication boundary instead of the shorter browser-console TTL, and known normal expiry exits cleanly with fresh-session guidance instead of a generic HTTP 401 failure.
- Routed low-ambiguity conversational Agent registration to deterministic, non-authorizing local Hermes default-profile preparation with the exact slash confirmation visible, and routed complete Registry count/list/show requests through authenticated Aegis authority instead of the untrusted model. Rich manager input now consumes complete navigation sequences at empty/history boundaries without exiting or clearing drafts, deletes UTF-8 runes safely on Backspace/Ctrl-H, and classifies actual interruption explicitly.
