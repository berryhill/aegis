...
- Made the generation-managed Badger store the exclusive production repository for mandates, authority contexts, lifecycle commands/facts, projections, and admission. Application composition now injects only narrow authority contracts; session start/end and broker use canonical activate/revoke/expire and fresh-admission paths; the generic filesystem authority implementation and duplicate tests were removed. Execution-session records remain a separate non-authoritative operational family.
...
- Made the typed layout rederive every state-rooted default after a state-directory override, and added one fail-closed clean-install classifier for legacy mandates, authority contexts, and authority revocations. Initialization permits only absent or securely proven empty legacy authority trees and revalidates immediately before apply; execution sessions remain separate operational metadata.
