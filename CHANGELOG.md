...
- Added a narrow internal fleet orchestration service that binds registration, Loop/Graph publication, and Graph submission to an authenticated subject and exact freshly admitted authority context; requires exact enabled participant and immutable definition bindings; durably rejects invalid submission admission; and reports mechanically distinct action-specific readiness for all consequential fleet actions. Publication consumes one validated authority resolution at its consequential boundary. CLI/API/runtime execution wiring, lifecycle activation, and the later queue/evidence lifecycle remain launch work.
- Added bounded canonical `internal/queue` submission, rejection, queue-item, claim/lease, and append-only initial-transition records plus `internal/execution` Graph-run, Loop-execution, and attempt records. The Badger `fleet-v1` adapter now atomically persists either an accepted snapshot/submission/queue-item/Graph-run/queued-transition bundle or a durable rejection under one outcome idempotency namespace, and provides a single-winner initial claim transaction that includes the attempt, claimed transition, and repository-assigned audit fact. Strict codecs, exact causal-reference checks, conflict denial, rollback tests, concurrent-claim tests, and close/reopen readback cover this internal substrate. These records grant no authority and are not yet wired to authenticated admission, runtime dispatch, later lease/retry/terminal lifecycle, CLI, HTTP, console, or installed end-to-end proof.
...
- Added the bounded, standard-library-only Loop definition domain with immutable typed revisions; deterministic canonical encoding and SHA-256 digests; strict decoding; validation for ports, steps, mappings, transitions, exclusive gates, joins, bounded retries, terminals, cycles, and evidence claims; and publication-time records binding the exact validator and validation digest. This layer grants no authority; durable immutable publication is available through the internal fleet adapter, while authenticated application services, execution, CLI, and HTTP routes remain absent.
- Added the bounded, standard-library-only Graph definition domain with immutable content-digested revisions; exact Agent, Loop, policy, and predecessor bindings; typed acyclic dependencies and mappings; strict deterministic codecs; immutable validation results; create-only contiguous publication checks; separate activation/retirement lifecycle; and content-digested run snapshots with canonical typed inputs and exact resolved definition references. The internal fleet adapter durably publishes these immutable facts and transaction-locally resolves snapshot Graph, validation, Agent, and Loop references. This layer grants no authority; authenticated publication, lifecycle application, queue admission, execution, CLI, and HTTP routes remain absent.
- Added a narrow Badger `fleet-v1` repository for atomic durable Registry registration/revisions, Loop and Graph revisions plus validation facts, Graph run snapshots, and repository-authored hash-linked audit facts. Exact publication and snapshot retries bind canonical request and audit semantics; changed same-key requests deny, failed transactions leave neither mutation nor audit, and clean close/reopen preserves verified facts. This is an internal persistence boundary, not authenticated admission or a completed Queue/execution loop.
- Added one standard-library-only shared reference layer for immutable identity/digest and identity/revision/digest bindings. Its deterministic versioned JSON codecs validate complete bindings and reject malformed identifiers or digests, unsupported schemas, zero revisions, unknown fields, duplicate object keys, invalid UTF-8, and trailing values; architecture tests enforce canonical ownership and inward-only dependency direction. References confer no authority; the Agent Registry and Graph definition domains consume them internally, while Execution Queue remains unimplemented.
