...
- Restored the authenticated gateway terminal as a real Aegis-agent conversation when its exact configured local model, certification, credential authority, Hermes runtime, Ollama route, and process custody are valid. Each gateway manager session now owns one clean disposable Hermes session bound to the authenticated principal, one `secrets-manager` stanza, and one bounded mandate; ordinary text uses the guarded manager ingress, while missing or invalid prerequisites remain a clearly labelled deterministic degraded shell with no cloud or alternate-model fallback.
