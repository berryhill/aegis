...
- Added the authenticated browser command-intent foundation under `/console/api/commands/preview` and `/console/api/commands/execute`: a closed versioned catalog, strict JSON/native-form request models, short-lived exact-target previews, server-derived subject/session/authority bindings, fresh execute-time authority and target admission, and exact duplicate readback. The catalog now contains the page-owned `loop.publish` and `loop.lifecycle` commands; unknown commands deny and additional commands still require a reviewed application service, authority adapter, and acceptance tests. Browser identity/authority/audit/success claims are rejected, and an uncertain commit is retained as terminal so cancellation, repository/audit failure, malformed receipt, or response retry cannot invoke a mutation twice.
...
- Promoted Agent Registry from a review-only console workspace to a principal-authenticated operational surface. Native same-origin CSRF-protected forms now review and register one exact immutable Agent revision from a pre-imported bounded canonical charter and deterministic current-fleet JSON, and append enabled/disabled/retired lifecycle revisions. Review is non-mutating; execute reparses and revalidates the artifacts before using the shared typed registration service, and lifecycle apply binds the exact current revision/digest. Every mutation reauthenticates the configured principal, preserves idempotency/audit contracts, and returns authoritative post-write readback; browser routes, display state, and prompt content grant no authority. Graphs, Loops, Queue, and Credentials keep their existing review/control boundaries.
...
- Superseded the former review-only Agent Registry primary action with the authenticated registration review/execute workflow. Registration now requires and validates an exact pre-imported canonical-charter revision, deterministic current-fleet JSON, and exact fleet/source identity; no charter import or provisioning operation is performed.
