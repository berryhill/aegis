...
- Added authenticated fresh/resumed bootstrap registration of the canonical immutable built-in `aegis` Agent (`aegis-system`, `manager-disposable`) with Aegis ownership, principal accountability, explicit default-decline approval, exact idempotent readback, and collision denial. Registration creates no persistent Hermes profile and grants no credential or model authority. Gateways already running from a pre-feature process are deliberately not silently backfilled while they own the stores; an explicitly approved gateway-owned backfill remains future work.
