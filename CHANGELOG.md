...
- Stopped fresh bare gateway activation from falling through into a terminal manager that may have no configured conversational model. Bare `aegis` now presents the password-gated browser console and returns; browser-launch failure reports the exact manual URL as an error. Explicit `aegis manager` and the healthy-gateway `terminal` action retain their authenticated conversational or truthful degraded behavior.
