...
- Corrected the Credentials workspace (#234): semantic Reference/Kind/Status/Version inventory, independently scrolling 42/58 selected desktop panes, and one visible pane at widths up to 900px. Detail retains reviewed-operation entry points, concise metadata, and immutable version history instead of vault/debug dossiers. Fragment selection navigates to the authenticated server-backed record query; per-history-entry presentation snapshots restore collection filters, selected-row focus, and inventory scroll. Native query-bearing links remain the no-script fallback; browser state grants no authority.
