...
- Aligned Registry toolbar, relationship chips, three-column card summaries, and inline detail header/readiness widths with the accepted design. Provisioning and recorded-session evidence use distinct inset groups, evaluated authority has its own result callout, and complete evidence identifiers wrap without overlapping adjacent values. Narrow Registry pages retain readable search and authentication controls without scaling a 390px viewport to overflow width; browser regressions assert the actual CSS viewport and expanded evidence layout. These presentation changes grant no authority.
- Rebuilt inline Agent detail into separate readiness, exact charter, historical provisioning receipts, immutable revision history, authenticated operator evaluation, complete stanza declarations, and session/execution evidence sections. Historical selection no longer replaces the current roster revision or exposes lifecycle mutation. Evaluation uses only the authenticated subject and trusted local environment with no requested stanza; missing evidence remains unavailable, and neither a match nor a receipt is presented as fresh runtime admission.
...
- Corrected architecture/threat-model custody descriptions and the current expertise projection version.
