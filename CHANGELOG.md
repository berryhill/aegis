...
- Added fresh/resumed offline bootstrap discovery and explicit import of the configured principal's existing default Hermes profile. After the sealed built-in `aegis` record is verified, Aegis now verifies an exact prior import or safely inspects canonical `~/.hermes/config.yaml`, presents separate default-decline read-only review and exact-registration decisions, and creates one immutable disabled `hermes-default-*` Agent only after digest-bound confirmation and full readback. Missing profile evidence remains non-gating; unsafe evidence and collisions fail closed; no profile contents, credentials, skills, memories, plugins, MCP state, model binding, capabilities, policies, activation, or ambient authority are imported.
