...
Principal-only administration is available through `aegis secret initialize|put|metadata|list|rotate|bind|revoke|backup`. The shared authority service also supplies bounded metadata list/search/history and the manager's closed proposal flow for create, rotate, revoke, and binding. A clear natural request such as `save my Google Drive key for person@example.com` is mapped by deterministic Aegis code to protected create metadata with safe defaults; operators do not need to name internal schema fields. If the sentence includes a quoted credential value, Aegis discards the complete message before history, audit, Hermes, or model context and continues only with derived metadata. Aegis renders the exact metadata-only preview, requires operator confirmation, and collects the value again through confirmed no-echo intake outside Hermes and Ollama. `put` and `rotate` retain deterministic CLI operation and protected `--stdin` support. Values are never accepted in argv or returned by the CLI. `metadata` and history do not decrypt. Host-file initialization is intended for development and is explicitly reported as weaker; production service configuration should use `LoadCredentialEncrypted` and `custody: systemd`. Keep KEK/recovery material separate from database backups.
