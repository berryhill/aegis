...
Badger authority-maintenance tests must use a disposable mode-`0700` authority root and backup directory. Cover the closed-clean prerequisite, context cancellation while an open store holds the shared lease, non-replacing mode-`0600` export, corrupt/truncated/oversized backup denial, fresh inactive import identity, exact activation and rollback, and fail-closed garbage collection that preserves active or unknown state. Never point export, import, activation, rollback, or garbage collection at a developer authority or retained backup.
