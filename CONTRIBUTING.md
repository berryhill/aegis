...
The command fails closed for a dirty tracked checkout or revision mismatch, constructs its candidate with inherited Git hooks disabled, applies the proposed release changelog transformation, and runs the full verification chain without publishing. For deeply nested checkouts, set `AEGIS_PROOF_SOCKET_DIR` to an existing canonical, non-symlink directory with a short absolute path under your durable proof workspace. Release readiness preserves that override for installed console Unix sockets; without it, the source repository remains the default. The installed console verifier still validates the directory; this override does not relax socket or runtime authority controls.
...
Run `python3 -m unittest scripts.console_browser_test_test` for focused Chrome harness regressions (requires `/usr/bin/google-chrome`). They require trusted touchstart/touchend/click delivery, native drawer activation after desktop/mobile transitions, and native anchor navigation with both an ordinary and a panned/zoomed visual viewport. Send one paired CDP touchStart/touchEnd sequence; do not substitute JavaScript activation or accept pointerup alone. Bounded passive document-event diagnostics retain measured coordinates and actual viewport geometry when target delivery fails. Keep DOM hit testing in layout CSS coordinates and CDP touch input in visual-viewport CSS coordinates; neither page scale nor device DPR is an extra multiplier. Request orderly Chrome shutdown and wait before removing its profile rather than ignoring cleanup errors.
