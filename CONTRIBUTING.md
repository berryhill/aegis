...
Root-dispatch, initialization, and updater tests must isolate `HOME`, `XDG_CONFIG_HOME`, state paths, and updater/network dependencies. They must never use the installed executable as an update target. Explicit configuration fixtures must be mode `0600`; insecure-permission tests must verify fail-closed behavior without rewriting the fixture.
...
The target validates exact stable SemVer, rejects non-`main`/unsynchronized/already-tagged state, moves pending changelog entries into the release, and verifies the committed source plus proposed release files in a disposable clone. Before changing the real repository, a non-dry-run release also creates and removes a signed preflight tag inside that disposable clone; pinentry, key, or signing failures therefore leave no release commit behind. Other unstaged work is left untouched and excluded from the release; pre-staged changes and pre-existing changelog edits are rejected to prevent accidental inclusion. Hermes performs an advisory review with only the in-session `todo` toolset and cannot approve or publish. Invoking `make release` is the operator's publication authorization; the deterministic script commits only the release files, creates a signed tag, and atomically pushes `main` with that tag. Use `RELEASE_DRY_RUN=1 make release` when publication is not intended.

The tag-triggered release workflow reruns formatting, tests, race tests, vet, and vulnerability checks; cross-builds Linux/macOS archives for amd64/arm64; verifies the embedded version and checksums; and creates the GitHub release. Tag creation and pushing are never delegated to Hermes.

Do not move or reuse a failed release tag. The public `v0.1.0` run failed in its test step before asset creation; the release-tag E2E fix landed afterward. Recovery therefore requires the maintainer to prepare and push the next SemVer release from fixed `main`. Until that external publication occurs, the release API correctly has no latest release and self-update must fail rather than fall back to an unverified tag artifact.
