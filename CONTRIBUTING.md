...
Root-dispatch, initialization, reset, and updater tests must isolate `HOME`, `XDG_CONFIG_HOME`, `XDG_STATE_HOME`, state, audit, checkpoint, authority, and KEK paths. They must never use the developer's real configuration, installed executable, Hermes profile, Ollama daemon/store, or external credentials as a target. Reset tests must exercise preview/apply identity drift, exact confirmation, non-TTY/cancellation no-write behavior, unknown-file preservation, and post-reset onboarding. Explicit configuration fixtures must be mode `0600`; insecure-permission tests must verify fail-closed behavior without rewriting the fixture.
