...
Run `python3 -m unittest scripts.console_browser_test_test` for focused Chrome harness regressions (requires `/usr/bin/google-chrome`). They require trusted touchstart/touchend/click delivery and native anchor navigation with both an ordinary and a panned/zoomed visual viewport. Keep DOM hit testing in layout CSS coordinates and CDP touch input in visual-viewport CSS coordinates; neither page scale nor device DPR is an extra multiplier. Request orderly Chrome shutdown and wait before removing its profile rather than ignoring cleanup errors.
