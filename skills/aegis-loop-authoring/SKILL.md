...
Use `loops implementation FILE` for bounded Go implementation semantics, not `loops example`. Supply strict JSON with `agent_id`, `loop_id`, positive `revision`, `idempotency_key`, optional `previous_digest`, and `implementation`: schema `aegis.loop.verified-implementation.v1`, bounded `task` and `acceptance`, explicit absolute `workspace`, local non-test Go `writable_files`, `max_passes` 1–2 (or 1–3 with exact `decision_mode: doer.v1`), and `policy` with kind `go-test.v1`, local `packages`, exact `required_tests` package/name identities and timeout 1–120 seconds. Obtain these values from the owner and existing trusted tests. Output is a v3 publication candidate; online validation must resolve the actual Agent. `--output` creates a new private file and refuses overwrite.

For Doer's selected-file one-task Loop, use `aegis loops doer FILE [--output NEW_FILE]`, not the Go-test-only implementation builder. The JSON supplies exact existing `agent_id`, `loop_id`, `revision`, `idempotency_key`, optional predecessor digest, and a `doer` object with bounded task, absolute operator-approved workspace, relative allowlisted `writable_files`, one matching `verify_file`, optional `expected_text` (absent means regular-file presence; explicit empty string asserts empty trimmed UTF-8), and `max_attempts` 1–3. The closed v4 step graph is Laya eligibility → bounded tool-free Hermes edits → Laya report judgment → independent selected-file check → terminal assessment or bounded diagnosis/retry. The builder prints the exact contract digest; publication remains an inactive definition and does not configure local Laya, select a model or authorize host writes. Execution requires separate operator-approved `implementation.authorized_contracts`, trusted `implementation.go_binary`, absolute `implementation.laya_python` and private offline `implementation.laya_home`, plus the normal Graph/Queue runtime authority. Never infer model availability from the skill or edit controller configuration merely to run a task. See `docs/DOER_LOOP.md` for the delivered bounded behavior and limits. It is not the upstream Doer skill's experimental model-family router or a generic multi-node Graph scheduler.
