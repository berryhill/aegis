...
3. Keep accepted and rejected outcomes distinct. The response envelope contains `accepted` or `rejection`, plus `created`. Acceptance contains an immutable snapshot, submission, queue item, Graph run, and initial transition. Workspace acceptance starts in `preparation-pending` (legacy `awaiting-runtime` remains readable), not runnable `queued`; it needs a fresh controller-issued runtime binding. A rejection contains a durable rejection identity, submission and idempotency identity, reason code, reason, and timestamp. Neither outcome may be relabeled by model narration.
