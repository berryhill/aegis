...
- **Unsupported:** Hermes is absent, its version is not a stable version satisfying `>=0.18.0`, or the exact local route is unavailable. Version acceptance does not establish future-release compatibility; protocol and tool/capability checks remain separate and fail closed. Install or repair separately under explicit operator control, then rerun inspection.
...
First Ctrl-C, SIGTERM, terminal EOF, exact `/quit`, exact `/exit`, exact `quit`, exact `exit`, session expiry, and revocation route through Aegis's bounded session cleanup lifecycle. A Hermes child, proxy, protocol, or conversational-turn failure instead invalidates and cleans only the disposable conversational route, records `manager_runtime_failed`, and leaves the authenticated deterministic manager session available until its existing mandate expires or is revoked. Do not submit another model turn through the failed runtime; `/status`, `/help agents`, `/agents readiness`, and `/agents list` remain deterministic controls. Cancellation is not a scanner-policy denial. Cleanup must report distinct runtime/session end and cleanup outcomes; do not claim cleanup merely because a process ended. A second Ctrl-C may force termination under the executable policy and is not graceful-completion evidence.
