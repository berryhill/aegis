...
The built-in manager boundary now owns terminal input, rejects bare non-terminal invocation, scans bounded source-aware content, strictly decodes one closed response/proposal envelope, and includes a session-token/route-digest/exact-model loopback inference proxy that scans serialized requests and bounded responses in hermetic tests. Scanner panic, timeout, malformed protocol, unauthorized proxy calls, alternate models, public Ollama endpoints, and digest drift fail closed. This is not yet an activated production route: no real model certification has been performed and the complete Hermes/Ollama lifecycle is not wired into bare startup.
